...
from core.labpilot_reporter import LabPilotReporter
...
class SenseBridgeApp(ctk.CTk):
    ...
    def __init__(self):
        ...
        # ---- LabPilot Integration ----
        self.reporter = LabPilotReporter(
            base_url=os.environ.get("LABPILOT_URL", "http://localhost:3000"),
            student_id=os.environ.get("STUDENT_ID", None),
            student_name=os.environ.get("STUDENT_NAME", "Lab Student"),
        )
    ...
    def _on_core_ready(self, loading_widget):
        ...
        # Report session start to LabPilot
        exp_mgr = self.session.experiment_manager
        if exp_mgr and exp_mgr.exp_data:
            self.reporter.start_session(exp_mgr.exp_data)
    ...
    def _advance_step(self, first_run=False):
        ...
        if not first_run:
            ...
            # Report step completion to LabPilot
            step = exp_mgr.get_current_step()
            self.reporter.report_step_event(
                exp_mgr.current_step_index, "completed",
                step_title=step.get("id", "") if step else "",
                cv_verified=True,
            )
        ...
        if exp_mgr.is_complete():
            ...
            self.reporter.report_scores(understanding=85, safety=95, engagement=80)
            self.reporter.end_session("completed")
            return
        ...
        # Report new step started to LabPilot
        step = exp_mgr.get_current_step()
        self.reporter.report_step_event(
            exp_mgr.current_step_index, "started",
            step_title=step.get("id", "") if step else "",
        )
    ...
    def on_closing(self):
        ...
        # End LabPilot session
        self.reporter.end_session("abandoned")
